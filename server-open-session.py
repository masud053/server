...
class C2Server:
    def __init__(self, gui_callback):
        ...
        self.running = False
        self.sessions = {}
        self.device_map = load_devices()
        self.persisted_sessions = load_sessions()
        self.lock = threading.Lock()
        self.active_session = None
        self.callback = gui_callback

        # On startup, note: sessions in 'persisted_sessions' are not auto-reconnected
        # but you can implement reconnection logic if needed.
    ...
    def _register_device(self, session):
        ...
        with self.lock:
            ...
            old_sid = self.device_map.get(session.device_id)
            if old_sid and old_sid in self.sessions and old_sid != session.id:
                try:
                    self.sessions[old_sid].close()
                except Exception:
                    pass
            self.device_map[session.device_id] = session.id
            save_devices(self.device_map)
            # Save persistent session info
            self.persisted_sessions[session.device_id] = {
                "session_id": session.id,
                "addr": session.addr
            }
            save_sessions(self.persisted_sessions)
    ...
    def _remove_session(self, session_id):
        with self.lock:
            ...
            if device_id:
                ...
                if device_id in self.persisted_sessions:
                    del self.persisted_sessions[device_id]
                    save_sessions(self.persisted_sessions)
                # Remove device map
            ...
            if session_id in self.sessions:
                ...
                if self.active_session == session_id:
                    # Keep the session alive unless device uninstalls
                    # For now, do not change active_session
                    if self.sessions:
                        self.active_session = next(iter(self.sessions))
                        self.callback("", {"type": "session_activate", "session_id": self.active_session})
                    else:
                        self.active_session = None
    ...
    def send_to_session(self, session_id, command, args=None):
        with self.lock:
            if session_id not in self.sessions:
                return False
            self.sessions[session_id].send({"type": "command", "command": command, "args": args or {}})
            return True
...
class C2GUI:
    def __init__(self, root):
        ...
        self.BTN_OPTS = {"width": 14}
        self.BTN_PACK = {"side": tk.LEFT, "padx": 1, "pady": 1}
    ...
    def _save_video(self):
        ...
        filename = os.path.join(self.save_dir, "video", f"{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4")
        video_writer = cv2.VideoWriter(filename, fourcc, 30.0, (640, 480))
        for frame_b64 in self.video_frames:
            try:
